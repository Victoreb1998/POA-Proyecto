...
/**
 * Clase que modela el protocolo de añadir un comprador en la parte del
 * responder
 *
 */
@SuppressWarnings("serial")
public class OpenBuyerCreditProtocolResponder extends AchieveREResponder {
	 ...
	 */
	public OpenBuyerCreditProtocolResponder(Agent a, MessageTemplate mt) {
		...
		try {
			...
			fishMarket.getLogger().info("ERROR",
					"The agent is not an instance of class that we expect (" + FishMarketAgent.class.getName() + ").");
			e.printStackTrace();
			throw e;
		}
	}
	...
	/**
	 * Metodo que prepara la respuesta a la petición. En caso de acceder a la
	 * petición se obvia el AGREE sino se manda un REFUSE.
	 * 
	...
	protected ACLMessage handleRequest(ACLMessage request) throws NotUnderstoodException, RefuseException {
		...
		fishMarket.getLogger().info("INFO", fishMarket.getLocalName()
				+ ": REQUEST to open a credit to a buyer received from " + request.getSender().getLocalName());
		// Comprobamos si podemos llevar a cabo la peticion.
		if (fishMarket.checkActionOpenCreditProtocol(request.getContent())) {
	...
	/**
	 * Metodo que lleva a cabo la petición propuesta y se envía un INFORM.
	 * 
	 * @param request  El mensaje recibido.
	...
	protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException {
		...
		if (fishMarket.performActionOpenCreditProtocol(request.getSender(), request.getContent())) {
			...
			fishMarket.getLogger().info("INFO", fishMarket.getLocalName() + ": Action successfully performed for "
					+ request.getSender().getLocalName() + " [OpenCreditProtocol]" + "Credit:" + request.getContent());
			...
			fishMarket.getLogger().info("INFO", fishMarket.getLocalName() + ": Action failed for "
					+ request.getSender().getLocalName() + " [OpenCreditProtocol]");
