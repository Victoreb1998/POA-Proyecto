...
public class AddSellerProtocolResponder extends AchieveREResponder 
{
	 ...
	 */
	public AddSellerProtocolResponder(Agent a, MessageTemplate mt) {
	...
	protected ACLMessage handleRequest(ACLMessage request) throws NotUnderstoodException, RefuseException {
		...
		System.out.println();
		fishMarket.getLogger().info("INFO",fishMarket.getLocalName() + ": REQUEST to admit a seller received from " + request.getSender().getLocalName());
		// Comprobamos si podemos llevar a cabo la peticion.
		if (fishMarket.checkActionAddSellerProtocol(request.getSender())) {
	...
	protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException {
		...
		if (fishMarket.performActionAddSellerProtocol(request.getSender())) 
		{
			// La peticion se ha realizado con exito.
			fishMarket.getLogger().info("INFO",fishMarket.getLocalName() + ": Action successfully performed for " + request.getSender().getLocalName() + " [AddSellerProtocol]");
			...
			fishMarket.getLogger().info("INFO",fishMarket.getLocalName() + ": Action failed for " + request.getSender().getLocalName() + " [AddSellerProtocol]");
